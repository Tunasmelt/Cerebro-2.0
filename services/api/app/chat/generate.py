...
Stage 7.13 — explicit generation config (CORRECTED post-launch). The
original pass here set a `safety_settings` field, "confirmed live"
against what turned out to be the wrong docs (a WebFetch pass pulled
schema from the Gemini *Enterprise Agent Platform's* Interactions API
reference, a same-shaped but distinct product from the consumer
Gemini API this codebase actually calls — the two were blended without
that being caught before merge). Caught live in production: every real
chat turn started failing with `{"error":{"message":"The parameter
'safety_settings' is not available on the Gemini API but it is
available on the Gemini Enterprise Agent Platform.","code":
"invalid_request"}}`. `safety_settings` removed entirely as a result —
not "fixed" to a different shape, removed, since this pass no longer
has confidence in the earlier "confirmed live" claim for *anything*
about this endpoint's config shape that wasn't independently re-
verified. `generation_config`/`max_output_tokens` is left in place
because the production error named only `safety_settings` specifically
as the invalid parameter — but this is a lower-confidence claim than
it should be, flagged honestly rather than re-asserted as "confirmed."
See phases-and-gates.md's Stage 7.13 entry for the corrected record.
"""
import asyncio
import json
import logging
...
# `safety_settings` was here too until a live production error proved
# it belongs to a different product (Gemini Enterprise Agent Platform,
# not the consumer Gemini API) — removed entirely, see this module's
# docstring.
